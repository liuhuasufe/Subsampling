...
# IMSE result
IMSE_3 = array(0,dim = c(100,2,6))
for (i in 1:100){
  for (j in 1:6){
    IMSE_3[i,,j] = sim(n,T,domain,lambda,r[j],r0[j],sigma,y0,NV)
    print(c(i,j))
  }
}
save.image("result33_0809.RData")

mIMSE_3 = apply(IMSE_3,c(2,3),mean)
matplot(r,t(mIMSE_3),type = "l",xlab = "subsample size",ylab = "IMSE",col = 1:2,lty = 1:2,lwd = 2,main = "Scenario III")
