...
# IMSE result
IMSE_2 = array(0,dim = c(100,2,6))
for (i in 1:100){
  for (j in 1:6){
    IMSE_2[i,,j] = sim(n,T,domain,lambda,r[j],r0[j],sigma,y0,NV)
    print(c(i,j))
  }
}
save.image("result22_0809.RData")

mIMSE_2 = apply(IMSE_2,c(2,3),mean)
matplot(r,t(mIMSE_2),type = "l",xlab = "subsample size",ylab = "IMSE",col = 1:2,lty = 1:2,lwd = 2,main = "Scenario II")
